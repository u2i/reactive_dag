...
| Drain loop | `ReactiveDag.Drain` | depth-ordered incremental propagation; `run/2` parameterized by the two seams, returning `{:ok, %Drain.Report{}}` — the processing trace (per-step cell/claimed/changed/`triggered_by`/`duration_us` + totals). Emits `:telemetry` events (`start`/`step`/`stop`/`exception`) so a dashboard, a metrics backend and a log can observe a drain independently; `step` carries the changed KEYS, which is what lets a consumer refresh only what moved. |
